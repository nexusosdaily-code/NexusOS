...
async function run() {
  ...
  const updateResult = await octokit.repos.createOrUpdateFileContents({
    ...
    repo: 'NexusOS',
    path: 'README.md',
    message: "docs: Theory of Compression States — first unobserved wavefunction and its evolution\n\nReplace Lambda Boson references with accurate physics: Einstein's Λ applied\nto the first oscillation, compression states from Planck floor to 10^120,\nfree quarks as first consolidation, spacetime as the evolving wavefunction,\nand the derivation of WNSP addressing from compression states.",
    content: Buffer.from(readme).toString('base64'),
    sha: fileData.sha
  });

  console.log('README updated:', updateResult.data.content.html_url);
...
run().catch(e => console.error('Error:', e.message, e.status));
