...
- If a branded microsite is reachable on both the bare host and the `www.` host, both variants need the same server-side metadata and AI-readable fallback HTML. Do not assume the bare-host `DOMAIN_META` entry will automatically cover `www.`.
