...
import { ArrowLeft, Activity, Globe, Bot, AlertTriangle, Radio, RefreshCw } from "lucide-react";
...
export default function NexusAnalyticsPage() {
  const [tab, setTab]       = useState<"pages" | "bots" | "countries" | "seo" | "live">("pages");
        ...
        {data && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard icon={AlertTriangle} label="SEO Issues" value={data.seoIssues.length} sub="Pages with 404s" color={data.seoIssues.length > 0 ? "red" : "green"} />
            </div>

            {/* Tabs */}
            <div className="flex gap-1 border-b border-slate-800">
              {(["pages","bots","countries","seo","live"] as const).map(t => (
                <button key={t} onClick={() => setTab(t)}
                  className={`px-4 py-2 text-xs font-mono capitalize transition-all border-b-2 -mb-px ${
                    tab === t
                      ? "border-cyan-500 text-cyan-300"
                      : "border-transparent text-slate-500 hover:text-slate-300"
                  }`}>
                  {t === "seo" ? "SEO Issues" : t === "live" ? "Live Feed" : t}
                </button>
              ))}
            </div>
              ...
              <div className="space-y-2">
                ...
                {data.topBots.length > 0 && (
                  <p className="text-[11px] text-slate-600 pt-2">
                    All bots listed hit authenticated endpoints and received 401 responses.
                    No data was exposed.
                  </p>
                )}
              </div>
            ...
            {tab === "seo" && (
              <div className="space-y-3">
                ...
                {data.seoIssues.length > 0 && (
                  <p className="text-[11px] text-slate-600 pt-2">
                    These paths received 404 responses from real visitors or crawlers.
                    Each one is a missed indexing opportunity.
                  </p>
                )}
              </div>
            )}
              ...
              <div className="space-y-1">
                ...
                {data.recentHits.map((h, i) => (
                  <div key={i} className={`grid grid-cols-[4rem_1fr_3rem_4rem_5rem] gap-2 px-3 py-2 rounded-lg text-xs items-center ${
                    h.isBot ? "bg-amber-500/5 border border-amber-500/10" : "bg-slate-900/20 border border-slate-800/40"
                  }`}>
                    ...
                    <span className={`text-[10px] truncate ${h.isBot ? "text-amber-400" : "text-green-400"}`}>
                      {h.isBot ? (h.botName ?? "Bot") : "Human"}
                    </span>
                  </div>
                ))}
