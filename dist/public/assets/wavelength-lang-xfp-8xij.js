import{j as e}from"./vendor-query-Bv9H5-RN.js";import{r as j,L as W}from"./vendor-router-tr6KpmeX.js";import{u as G}from"./use-page-meta-BRxgrg9H.js";import{f as z,C as P,I as B,Z as H,D as _,d as R,ap as q,b as I,R as D,B as F,y as K}from"./vendor-icons-B5ISShIx.js";function E(d){return d<450?"#8b00ff":d<495?"#2563eb":d<520?"#06b6d4":d<565?"#16a34a":d<590?"#ca8a04":d<625?"#ea580c":"#dc2626"}function $(d){return d<450?"SYSTEM":d<495?"AUTH":d<520?"STREAM":d<565?"LOGIC":d<590?"INTERFACE":d<625?"EVENT":"STORAGE"}function g(d){const b=d.toUpperCase().split("").map(m=>m.charCodeAt(0)).filter(m=>m>=32&&m<=126);b.length||b.push(77);const f=b.reduce((m,n)=>m+n,0)/b.length,o=parseFloat((380+(f-32)/94*400).toFixed(2)),c=parseFloat((299792458/(o*1e-9)/1e12).toFixed(2)),w=Math.floor((o-380)/4)+1,s=b.reduce((m,n)=>m+n,0)%100,S=b.length%2===0?"H":"V";return{nm:o,thz:c,psi:`Ψ(${w},${s},${S})`,band:$(o)}}function U(d,b){if(!d.trim())return"";const f=d.split(`
`),o=["// ── WavelengthScript v1.0 · AGPL-3.0 · NexusOS ─────────────────",`// Source: ${b.toUpperCase()} → WLS transpilation`,`// Generated: ${new Date().toISOString()}`,""];for(const w of f){const s=w.trim();if(!s){o.push("");continue}if(s.startsWith("#")||s.startsWith("//")){o.push(`// ${s.replace(/^[#/]+\s*/,"")}`);continue}const S=s.match(/^(?:def|function|fn)\s+(\w+)\s*\(([^)]*)\)/);if(S){const[,l,h]=S,p=g(l),x=h.split(",").map(v=>v.trim()).filter(Boolean).map(v=>`@${g(v.replace(/[^a-zA-Z]/g,"")||"x").nm}nm ${v.trim()}`).join(", ");o.push(`@emit(${p.nm}nm, ${p.psi}) // λ=${p.nm}nm · ${p.band}`),o.push(`fn ${l}(${x}) {`);continue}const m=s.match(/^(?:class|struct)\s+(\w+)/);if(m){const l=g(m[1]);o.push(`@channel(${l.psi}) // ${l.nm}nm · ${l.band}`),o.push(`type ${m[1]} : SpectralNode {`);continue}const n=s.match(/^(?:let|const|var)?\s*(\w+)\s*[:=]+\s*(.+)/);if(n){const[,l,h]=n,p=g(l);o.push(`@${p.nm}nm let ${l} := ${h.replace(/;$/,"")}  // ${p.psi}`);continue}if(s.startsWith("return")){o.push(`  emit ${s.slice(6).trim()}  // → spectral output`);continue}if(s.startsWith("import")||s.startsWith("use")||s.startsWith("require")){const l=s.match(/["']([^"']+)["']/),h=l?l[1]:"module",p=g(h.replace(/[^a-zA-Z]/g,"")||"mod");o.push(`tune(${p.nm}nm)  // import ${h} at ${p.psi}`);continue}if(s.match(/^(?:print|console\.log|println!|System\.out)/)){o.push(`  broadcast(${s.replace(/^[^(]+/,"")})  // → 520nm STREAM band`);continue}if(s.startsWith("if ")||s==="else"||s.startsWith("else")){o.push(`  ?λ ${s.replace(/^else\s*/,"// else ")}:`);continue}if(s.startsWith("for ")||s.startsWith("while ")){o.push(`  oscillate(${s.replace(/^(for|while)\s+/,"")}) {`);continue}if(s==="}"||s==="}"||s.match(/^end(\s|$)/)){o.push("}");continue}const i=g(s.split(" ")[0]||"op");o.push(`  /* @${i.nm}nm */ ${s}`)}o.push(""),o.push("// ── Spectral manifest ───────────────────────────────────────────");const c=Array.from(new Set(d.match(/\b[a-zA-Z_][a-zA-Z0-9_]{2,}\b/g)??[])).slice(0,8);for(const w of c){const s=g(w);o.push(`// ${w.padEnd(20)} → ${s.nm}nm  ${s.psi}  [${s.band}]`)}return o.join(`
`)}function Z(d){if(!d.trim())return{assembly:"",hex:"",manifest:[],instrCount:0};const b=[],f=new Map;let o=0;function c(n,i,l,h,p,x){b.push({off:o,op:n,mnem:i,args:l,nm:p,ch:x,cmt:h}),n!==0&&(o+=8)}c(0,".WNSP","v1.0","NexusOS WNSP Bytecode · AGPL-3.0"),c(0,".ARCH","WDM256·OAM50·POL2·DIR2","51,200 orthogonal Ψ channels"),c(0,".MODEL","Λ=hf/c² SPECTRAL","Einstein first-principle execution"),c(0,"","",""),o=0;for(const n of d.split(`
`)){const i=n.trim();if(!i){c(0,"","","");continue}if(i.startsWith("//")||i.startsWith(";")||i.startsWith("#")){c(0,";",i.replace(/^[/;#]+\s*/,""),"");continue}const l=i.match(/@emit\((\d+\.?\d*)nm,\s*(Ψ\([^)]+\))\)/);if(l){const a=parseFloat(l[1]);f.set(l[2],{nm:a,psi:l[2],band:$(a)}),c(3,"EMIT",`λ=${a}nm  ${l[2]}`,`emit on ${$(a)} band`,a,l[2]);continue}const h=i.match(/tune\((\d+\.?\d*)nm\)/);if(h){const a=parseFloat(h[1]);c(1,"TUNE",`λ=${a}nm`,`receiver → ${$(a)} band`,a);continue}const p=i.match(/^agent\s+(\w+)/);if(p){const a=g(p[1]);f.set(p[1],a),c(10,"AGENT",`"${p[1]}"  ${a.psi}`,`AI agent λ=${a.nm}nm · ${a.band}`,a.nm,a.psi);continue}const x=i.match(/^fn\s+(\w+)/);if(x){const a=g(x[1]);f.set(x[1],a),c(7,"LABEL",`${x[1]}  ${a.psi}`,`fn → λ=${a.nm}nm`,a.nm,a.psi);continue}const v=i.match(/node\.register\("([^"]+)"/);if(v){const a=g(v[1]);f.set(v[1],a),c(10,"AGENT",`"${v[1]}"  ${a.psi}  PUBLIC`,"spectral network node",a.nm,a.psi);continue}const u=i.match(/oscillate\(([^)]+)\)/);if(u){c(6,"OCS",u[1].trim(),"non-blocking wave loop");continue}const A=i.match(/broadcast\(([^)]+)\)/);if(A){const a=g(A[1].replace(/[^a-zA-Z]/g,"")||"data");c(5,"BROAD",A[1].trim(),`broadcast λ=${a.nm}nm`,a.nm);continue}const k=i.match(/@(\d+\.?\d*)nm\s+let\s+(\w+)\s*:=/);if(k){const a=parseFloat(k[1]);f.set(k[2],{nm:a,psi:g(k[2]).psi,band:$(a)}),c(2,"PUSH",`@${a}nm  "${k[2]}"`,`bind at λ=${a}nm · ${$(a)}`,a);continue}const C=i.match(/^\s*emit\s+(.+)/);if(C){const a=g(C[1].replace(/[^a-zA-Z]/g,"")||"out");c(3,"EMIT",C[1].trim(),`output at λ=${a.nm}nm`,a.nm);continue}if(i.startsWith("?λ ")){c(8,"JMPZ",i.slice(3).trim(),"photon path branch");continue}if(i==="}"||i.match(/^end\b/)){c(254,"RET","","scope end — wave collapses");continue}const y=i.split(/\s/)[0].replace(/[^a-zA-Z]/g,"")||"op",T=g(y);c(11,"EXEC",`@${T.nm}nm`,i.slice(0,50),T.nm)}c(255,"HALT","","wavefunction terminated");const w=[...f.entries()].map(([n,i])=>({symbol:n,...i})),s=b.filter(n=>n.op!==0&&n.mnem!==""&&n.mnem!==";"),S=["; ── WNSP Bytecode Assembly v1.0 ────────────────────────────────────",`; NexusOS · AGPL-3.0 · ${new Date().toISOString().slice(0,19)}Z`,"; Hilbert-space: 51,200 orthogonal channels · E=hf · Λ=hf/c²","; ────────────────────────────────────────────────────────────────────","",...b.map(n=>{if(!n.mnem)return"";if(n.mnem===";")return`  ; ${n.args}`;if(n.mnem.startsWith("."))return`${n.mnem.padEnd(10)} ${n.args}  ; ${n.cmt}`;const i=`0x${n.off.toString(16).padStart(6,"0")}`,l=n.op.toString(16).padStart(2,"0"),h=n.cmt?`  ; ${n.cmt}`:"";return`  ${i}  ${l}  ${n.mnem.padEnd(8)} ${n.args}${h}`}),"",`; ── Symbol Table (${w.length} symbols) ─────────────────────────────────────`,...w.map(n=>`; ${n.symbol.padEnd(22)} λ=${String(n.nm).padEnd(8)} ${n.psi}  [${n.band}]`)],m=["; WNSP Binary Hex Dump","; ─────────────────────────────────────────────────────────────","Offset    Bytes                              Annotation","────────  ─────────────────────────────────  ──────────────────",'0x000000  57 4E 53 50 01 00 00 00            ; magic "WNSP" v1.0',`0x000008  ${s.length.toString(16).padStart(8,"0").match(/.{2}/g).join(" ")}            ; instr count = ${s.length}`,...s.slice(0,12).map((n,i)=>{const l=`0x${(16+i*8).toString(16).padStart(6,"0")}`,h=n.op.toString(16).padStart(2,"0"),p=n.nm?Math.round(n.nm*10):0,x=Math.floor(p/256).toString(16).padStart(2,"0"),v=(p%256).toString(16).padStart(2,"0");return`${l}  ${h} ${x} ${v} 00 00 00 00 00 ; ${n.mnem.padEnd(6)} ${n.args.slice(0,24)}`}),s.length>12?`...  (${s.length-12} more instructions)`:""];return{assembly:S.join(`
...
`,O=`// WavelengthScript v1.0 · AGPL-3.0
