...
;
  ...
  // expiry.  The opposite extreme pins the survival guarantee at the youngest
  // conceivable cooldown: lastAlerted === pruneNow, giving an age of exactly 0.
  //
  //   now - entry.lastAlerted = pruneNow - pruneNow = 0 < COOLDOWN_MS  → hasActiveCooldown = true
  //
...
;
  ...
  // These two tests pin the boundary one step away from the edge:
  //
  //   lastAlerted = pruneNow − 1  →  age = 1 ms
  //   now − entry.lastAlerted = 1  <  COOLDOWN_MS  →  hasActiveCooldown = true
  //
  // The entry must SURVIVE because the strict-less-than operator `<` includes
  // age = 1.  A future refactor that accidentally uses `<=` with
  // `COOLDOWN_MS − 1`, or any other form that excludes age = 1, would be
  // caught here even if B45/B46 (age = 0) happened to pass.
  //
  // Design (B50 — UA map):
  //   lastAlerted = pruneNow − 1   (age = 1 ms) → 1 < COOLDOWN_MS → hasActiveCooldown = true
  //   hits[last]  = pruneNow − WINDOW_MS − 1     → below cutoff   → hasActiveHits     = false
  //   ⇒ !hasActiveHits && !hasActiveCooldown is FALSE → SURVIVE
  //
  ...
  it("(B50) zombie UA entry with lastAlerted === pruneNow − 1 (age = 1 ms) and all stale hits SURVIVES the prune pass", async () => {
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B50-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry 1 ms inside the cooldown window:
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − 1 → age = 1 ms → hasActiveCooldown true
    const KEY = "B50ZombieOneMsCooldownUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],  // 1 ms outside the window (stale)
      lastAlerted: pruneNow - 1,                // age = 1 ms, inside COOLDOWN_MS
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B50-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = 1 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B51) zombie referer entry with lastAlerted === pruneNow − 1 (age = 1 ms) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B50.
    //
    // T0 = Date.now()+150h — monotonically above B50 (148h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 150 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b51-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry 1 ms inside the cooldown window:
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − 1 → age = 1 ms → hasActiveCooldown true
    const KEY = "https://b51-zombie-one-ms-cooldown.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],  // 1 ms outside the window (stale)
      lastAlerted: pruneNow - 1,                // age = 1 ms, inside COOLDOWN_MS
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b51-stale-companion.example/scan")).toBe(false);
  ...
  // age that still satisfies `now − lastAlerted < COOLDOWN_MS`).
  // B35/B36 pinned exact-expiry eviction: age = COOLDOWN_MS (the first age
  // where the guard evaluates to false).
  //
  // The gap between those two is the UPPER survival boundary:
  //   lastAlerted = pruneNow − (COOLDOWN_MS − 1)  →  age = COOLDOWN_MS − 1
  //   now − entry.lastAlerted = COOLDOWN_MS − 1  <  COOLDOWN_MS  →  hasActiveCooldown = true
  //
  // The entry must SURVIVE because age = COOLDOWN_MS − 1 is still strictly
  // less than COOLDOWN_MS.  A future refactor that tightens the guard to
  // `< COOLDOWN_MS − 1` (or uses `<= COOLDOWN_MS − 2`) would evict this
  // entry while leaving B35/B36 and B50/B51 both passing — this test closes
  // that gap.
  //
  // Design (B52 — UA map):
  //   lastAlerted = pruneNow − (COOLDOWN_MS − 1)  → age = COOLDOWN_MS−1 < COOLDOWN_MS → hasActiveCooldown = true
  //   hits[last]  = pruneNow − WINDOW_MS − 1       → below cutoff                     → hasActiveHits     = false
  //   ⇒ !hasActiveHits && !hasActiveCooldown is FALSE → SURVIVE
  //
  // T0 = Date.now()+152h (B52) and +154h (B53) — monotonically above B51 (150h).

  it("(B52) zombie UA entry with lastAlerted === pruneNow − (COOLDOWN_MS − 1) (age = COOLDOWN_MS−1) and all stale hits SURVIVES the prune pass", async () => {
    const mod = await import("./traffic-logger");
    const { _uaProbes, _refererProbes, _pruneProbes } = mod as any;
    ...
    // T0 = Date.now()+152h — monotonically above B51 (150h).
    const T0       = Date.now() + 152 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B52-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry one millisecond before cooldown expiry (upper survival boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 1) → age = COOLDOWN_MS−1 → hasActiveCooldown true
    const KEY = "B52ZombieUpperCooldownBoundaryUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 1),        // age = COOLDOWN_MS−1, still inside cooldown
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B52-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = COOLDOWN_MS−1 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B53) zombie referer entry with lastAlerted === pruneNow − (COOLDOWN_MS − 1) (age = COOLDOWN_MS−1) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B52.
    //
    // T0 = Date.now()+154h — monotonically above B52 (152h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 154 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b53-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry one millisecond before cooldown expiry (upper survival boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 1) → age = COOLDOWN_MS−1 → hasActiveCooldown true
    const KEY = "https://b53-zombie-upper-cooldown-boundary.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 1),        // age = COOLDOWN_MS−1, still inside cooldown
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b53-stale-companion.example/scan")).toBe(false);
  ...
  // ── B54 / B55: one step inside the upper cooldown-survival boundary (age = COOLDOWN_MS − 2) ──
  //
  // B52/B53 pin the UPPER survival boundary at age = COOLDOWN_MS − 1.  A future
  // refactor that tightens the cooldown guard to `< COOLDOWN_MS − 1` (or uses
  // `<= COOLDOWN_MS − 2`) would evict an entry at age = COOLDOWN_MS − 2 while
  // still passing B52/B53 (which only test age = COOLDOWN_MS − 1 exactly).
  //
  // These tests add one step further inside the window to close that gap:
  //   lastAlerted = pruneNow − (COOLDOWN_MS − 2)  →  age = COOLDOWN_MS − 2
  //   now − entry.lastAlerted = COOLDOWN_MS − 2  <  COOLDOWN_MS  →  hasActiveCooldown = true
  //
  // The entry must SURVIVE because age = COOLDOWN_MS − 2 is strictly less than COOLDOWN_MS.
  //
  // T0 = Date.now()+156h (B54) and +158h (B55) — monotonically above B53 (154h).

  it("(B54) zombie UA entry with lastAlerted === pruneNow − (COOLDOWN_MS − 2) (age = COOLDOWN_MS−2) and all stale hits SURVIVES the prune pass", async () => {
    const mod = await import("./traffic-logger");
    const { _uaProbes, _refererProbes, _pruneProbes } = mod as any;
    ...
    // T0 = Date.now()+156h — monotonically above B53 (154h).
    const T0       = Date.now() + 156 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B54-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry two milliseconds before cooldown expiry (one step inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 2) → age = COOLDOWN_MS−2 → hasActiveCooldown true
    const KEY = "B54ZombieInsideUpperCooldownBoundaryUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 2),        // age = COOLDOWN_MS−2, still inside cooldown
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B54-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = COOLDOWN_MS−2 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B55) zombie referer entry with lastAlerted === pruneNow − (COOLDOWN_MS − 2) (age = COOLDOWN_MS−2) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B54.
    //
    // T0 = Date.now()+158h — monotonically above B54 (156h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 158 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b55-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry two milliseconds before cooldown expiry (one step inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 2) → age = COOLDOWN_MS−2 → hasActiveCooldown true
    const KEY = "https://b55-zombie-inside-upper-cooldown-boundary.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 2),        // age = COOLDOWN_MS−2, still inside cooldown
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b55-stale-companion.example/scan")).toBe(false);
  ...
  // ── B58 / B59: three steps inside the upper cooldown-survival boundary (age = COOLDOWN_MS − 4) ──
  //
  // B56/B57 pin a survival boundary at age = COOLDOWN_MS − 3.  A future refactor
  // that tightens the cooldown guard to `< COOLDOWN_MS − 3` (or uses
  // `<= COOLDOWN_MS − 4`) would evict an entry at age = COOLDOWN_MS − 4 while
  // still passing B56/B57 (which only test age = COOLDOWN_MS − 3 exactly).
  //
  // These tests add one step further inside the window to close that gap:
  //   lastAlerted = pruneNow − (COOLDOWN_MS − 4)  →  age = COOLDOWN_MS − 4
  //   now − entry.lastAlerted = COOLDOWN_MS − 4  <  COOLDOWN_MS  →  hasActiveCooldown = true
  //
  // The entry must SURVIVE because age = COOLDOWN_MS − 4 is strictly less than COOLDOWN_MS.
  //
  // T0 = Date.now()+164h (B58) and +166h (B59) — monotonically above B57 (162h).

  it("(B58) zombie UA entry with lastAlerted === pruneNow − (COOLDOWN_MS − 4) (age = COOLDOWN_MS−4) and all stale hits SURVIVES the prune pass", async () => {
    const mod = await import("./traffic-logger");
    const { _uaProbes, _refererProbes, _pruneProbes } = mod as any;
    ...
    // T0 = Date.now()+164h — monotonically above B57 (162h).
    const T0       = Date.now() + 164 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B58-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry four milliseconds before cooldown expiry (three steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 4) → age = COOLDOWN_MS−4 → hasActiveCooldown true
    const KEY = "B58ZombieInsideUpperCooldownBoundaryUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 4),        // age = COOLDOWN_MS−4, still inside cooldown
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B58-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = COOLDOWN_MS−4 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B59) zombie referer entry with lastAlerted === pruneNow − (COOLDOWN_MS − 4) (age = COOLDOWN_MS−4) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B58.
    //
    // T0 = Date.now()+166h — monotonically above B58 (164h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 166 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b59-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry four milliseconds before cooldown expiry (three steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 4) → age = COOLDOWN_MS−4 → hasActiveCooldown true
    const KEY = "https://b59-zombie-inside-upper-cooldown-boundary.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 4),        // age = COOLDOWN_MS−4, still inside cooldown
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b59-stale-companion.example/scan")).toBe(false);
  ...
  // ── B60 / B61: four steps inside the upper cooldown-survival boundary (age = COOLDOWN_MS − 5) ──
   //
   // B58/B59 pin a survival boundary at age = COOLDOWN_MS − 4.  A future refactor
   // that tightens the cooldown guard to `< COOLDOWN_MS − 4` (or uses
   // `<= COOLDOWN_MS − 5`) would evict an entry at age = COOLDOWN_MS − 5 while
   // still passing B58/B59 (which only test age = COOLDOWN_MS − 4 exactly).
   //
   // These tests add one step further inside the window to close that gap:
   //   lastAlerted = pruneNow − (COOLDOWN_MS − 5)  →  age = COOLDOWN_MS − 5
   //   now − entry.lastAlerted = COOLDOWN_MS − 5  <  COOLDOWN_MS  →  hasActiveCooldown = true
   //
   // The entry must SURVIVE because age = COOLDOWN_MS − 5 is strictly less than COOLDOWN_MS.
   //
   // T0 = Date.now()+168h (B60) and +170h (B61) — monotonically above B59 (166h).

  it("(B60) zombie UA entry with lastAlerted === pruneNow − (COOLDOWN_MS − 5) (age = COOLDOWN_MS−5) and all stale hits SURVIVES the prune pass", async () => {
    const mod = await import("./traffic-logger");
    const { _uaProbes, _refererProbes, _pruneProbes } = mod as any;
    ...
    // T0 = Date.now()+168h — monotonically above B59 (166h).
    const T0       = Date.now() + 168 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B60-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry five milliseconds before cooldown expiry (four steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 5) → age = COOLDOWN_MS−5 → hasActiveCooldown true
    const KEY = "B60ZombieInsideUpperCooldownBoundaryUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 5),        // age = COOLDOWN_MS−5, still inside cooldown
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B60-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = COOLDOWN_MS−5 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B61) zombie referer entry with lastAlerted === pruneNow − (COOLDOWN_MS − 5) (age = COOLDOWN_MS−5) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B60.
    //
    // T0 = Date.now()+170h — monotonically above B60 (168h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 170 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b61-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry five milliseconds before cooldown expiry (four steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 5) → age = COOLDOWN_MS−5 → hasActiveCooldown true
    const KEY = "https://b61-zombie-inside-upper-cooldown-boundary.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 5),        // age = COOLDOWN_MS−5, still inside cooldown
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b61-stale-companion.example/scan")).toBe(false);
   ...
   // ── B62 / B63: five steps inside the upper cooldown-survival boundary (age = COOLDOWN_MS − 6) ──
    //
    // B60/B61 pin a survival boundary at age = COOLDOWN_MS − 5.  A future refactor
    // that tightens the cooldown guard to `< COOLDOWN_MS − 5` (or uses
    // `<= COOLDOWN_MS − 6`) would evict an entry at age = COOLDOWN_MS − 6 while
    // still passing B60/B61 (which only test age = COOLDOWN_MS − 5 exactly).
    //
    // These tests add one step further inside the window to close that gap:
    //   lastAlerted = pruneNow − (COOLDOWN_MS − 6)  →  age = COOLDOWN_MS − 6
    //   now − entry.lastAlerted = COOLDOWN_MS − 6  <  COOLDOWN_MS  →  hasActiveCooldown = true
    //
    // The entry must SURVIVE because age = COOLDOWN_MS − 6 is strictly less than COOLDOWN_MS.
    //
    // T0 = Date.now()+172h (B62) and +174h (B63) — monotonically above B61 (170h).

  it("(B62) zombie UA entry with lastAlerted === pruneNow − (COOLDOWN_MS − 6) (age = COOLDOWN_MS−6) and all stale hits SURVIVES the prune pass", async () => {
    const mod = await import("./traffic-logger");
    const { _uaProbes, _refererProbes, _pruneProbes } = mod as any;
    ...
    // T0 = Date.now()+172h — monotonically above B61 (170h).
    const T0       = Date.now() + 172 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B62-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry six milliseconds before cooldown expiry (five steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 6) → age = COOLDOWN_MS−6 → hasActiveCooldown true
    const KEY = "B62ZombieInsideUpperCooldownBoundaryUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 6),        // age = COOLDOWN_MS−6, still inside cooldown
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B62-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = COOLDOWN_MS−6 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B63) zombie referer entry with lastAlerted === pruneNow − (COOLDOWN_MS − 6) (age = COOLDOWN_MS−6) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B62.
    //
    // T0 = Date.now()+174h — monotonically above B62 (172h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 174 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b63-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry six milliseconds before cooldown expiry (five steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 6) → age = COOLDOWN_MS−6 → hasActiveCooldown true
    const KEY = "https://b63-zombie-inside-upper-cooldown-boundary.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 6),        // age = COOLDOWN_MS−6, still inside cooldown
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b63-stale-companion.example/scan")).toBe(false);
    ...
    // ── B64 / B65: six steps inside the upper cooldown-survival boundary (age = COOLDOWN_MS − 7) ──
     //
     // B62/B63 pin a survival boundary at age = COOLDOWN_MS − 6.  A future refactor
     // that tightens the cooldown guard to `< COOLDOWN_MS − 6` (or uses
     // `<= COOLDOWN_MS − 7`) would evict an entry at age = COOLDOWN_MS − 7 while
     // still passing B62/B63 (which only test age = COOLDOWN_MS − 6 exactly).
     //
     // These tests add one step further inside the window to close that gap:
     //   lastAlerted = pruneNow − (COOLDOWN_MS − 7)  →  age = COOLDOWN_MS − 7
     //   now − entry.lastAlerted = COOLDOWN_MS − 7  <  COOLDOWN_MS  →  hasActiveCooldown = true
     //
     // The entry must SURVIVE because age = COOLDOWN_MS − 7 is strictly less than COOLDOWN_MS.
     //
     // T0 = Date.now()+176h (B64) and +178h (B65) — monotonically above B63 (174h).

  it("(B64) zombie UA entry with lastAlerted === pruneNow − (COOLDOWN_MS − 7) (age = COOLDOWN_MS−7) and all stale hits SURVIVES the prune pass", async () => {
    const mod = await import("./traffic-logger");
    const { _uaProbes, _refererProbes, _pruneProbes } = mod as any;
    ...
    // T0 = Date.now()+176h — monotonically above B63 (174h).
    const T0       = Date.now() + 176 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran before checking the target.
    _uaProbes.set("B64-stale-companion-ua/1.0", { hits: [], lastAlerted: 0 });

    // Zombie UA entry seven milliseconds before cooldown expiry (six steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 7) → age = COOLDOWN_MS−7 → hasActiveCooldown true
    const KEY = "B64ZombieInsideUpperCooldownBoundaryUA/1.0";
    _uaProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 7),        // age = COOLDOWN_MS−7, still inside cooldown
    });

    _pruneProbes(pruneNow);

    // Stale companion deleted → prune loop ran.
    expect(_uaProbes.has("B64-stale-companion-ua/1.0")).toBe(false);

    // hasActiveCooldown is true (age = COOLDOWN_MS−7 < COOLDOWN_MS) → zombie must SURVIVE.
    expect(_uaProbes.has(KEY)).toBe(true);
  ...
  it("(B65) zombie referer entry with lastAlerted === pruneNow − (COOLDOWN_MS − 7) (age = COOLDOWN_MS−7) and all stale hits SURVIVES the prune pass", async () => {
    // Symmetric referer-map counterpart to B64.
    //
    // T0 = Date.now()+178h — monotonically above B64 (176h).
    const mod = await import("./traffic-logger");
    const { _refererProbes, _uaProbes, _pruneProbes } = mod as any;
    ...
    const T0       = Date.now() + 178 * 60 * 60 * 1000;
    const pruneNow = T0 + COOLDOWN_MS + 1;
    ...
    // Stale companion — confirms the prune loop ran.
    _refererProbes.set("https://b65-stale-companion.example/scan", { hits: [], lastAlerted: 0 });

    // Zombie referer entry seven milliseconds before cooldown expiry (six steps inside upper boundary):
    //   hits: single timestamp 1 ms past the window cutoff (stale → hasActiveHits false)
    //   lastAlerted: pruneNow − (COOLDOWN_MS − 7) → age = COOLDOWN_MS−7 → hasActiveCooldown true
    const KEY = "https://b65-zombie-inside-upper-cooldown-boundary.example/scan";
    _refererProbes.set(KEY, {
      hits:        [pruneNow - WINDOW_MS - 1],          // 1 ms outside the window (stale)
      lastAlerted: pruneNow - (COOLDOWN_MS - 7),        // age = COOLDOWN_MS−7, still inside cooldown
    });
    ...
    // Stale companion deleted → prune loop ran.
    expect(_refererProbes.has("https://b65-stale-companion.example/scan")).toBe(false);
  ...
  it("(B41c) initProbeCounters: allHits buffer is fresh per group — mutating one group's hits array does not affect another group's hits array", async () => 
{
  ...
  it("(B41i) dedup merge takes the highest lastAlerted across three UA rows when the highest value is in the middle row", async () => {
    // B41e and B41f only cover the two-row case, where a first-wins or
    // last-wins implementation still happens to pick the maximum (because with
    // two rows there are only two candidates).  A three-row seed where the
    // highest lastAlerted sits in the MIDDLE row (low / HIGH / low) exposes
    // both failure modes at once:
    //   • first-wins: returns low (row 0) ✗
    //   • last-wins:  returns low (row 2) ✗
    //   • Math.max:   returns HIGH       ✓
    //
    // T0 = Date.now()+148h — monotonically above B41h (146h) to avoid any
    // in-memory state left over from prior tests in the same describe block.

    const WINDOW_MS = 24 * 60 * 60 * 1000;
    const initNow   = Date.now() + 148 * 60 * 60 * 1000;
    // All three hits are inside the window so the entry is not skipped.
    const hit0 = initNow - Math.floor(WINDOW_MS / 3);
    const hit1 = initNow - Math.floor(WINDOW_MS / 4);
    const hit2 = initNow - Math.floor(WINDOW_MS / 5);

    // row 0 and row 2 have a LOWER lastAlerted; row 1 (the middle) has the HIGHEST.
    const lowerLastAlerted  = initNow - 60 * 60 * 1000; // 60 min ago
    const highestLastAlerted = initNow - 5 * 60 * 1000; // 5 min ago — must win
    const alsoLower         = initNow - 45 * 60 * 1000; // 45 min ago
    ...
    try {
      ...
      const KEY  = "B41iThreeRowMiddleHighUA/1.0";
      const row0 = { fieldType: "ua", key: KEY, hits: [hit0], lastAlerted: lowerLastAlerted };
      const row1 = { fieldType: "ua", key: KEY, hits: [hit1], lastAlerted: highestLastAlerted };
      const row2 = { fieldType: "ua", key: KEY, hits: [hit2], lastAlerted: alsoLower };
      ...
      // A first-wins implementation would return lowerLastAlerted (row 0).
      // A last-wins implementation would return alsoLower (row 2).
      // Only Math.max over all rows returns highestLastAlerted (row 1).
      expect(entry!.lastAlerted).toBe(highestLastAlerted);
    } finally {
      dateNowSpy.mockRestore();
    }
  });
  ...
  it("(B41h) initProbeCounters: allHits buffer is independent between two distinct UA keys when each key has exactly one DB row (no duplicates)", async () => {
    // Regression guard for the fresh-import variant of the shared-buffer bug.
    // B41f covers the case where each key has two duplicate DB rows.  This test
    // covers the simpler case where each key has a single DB row — no duplicates
    // at all.  A refactor that declares `allHits` outside the group loop would
    // cause the second group iteration to append into the buffer left over from
    // the first group, making both entries alias the same accumulated array.
    //
    // Setup: two distinct UA keys — KEY_P and KEY_Q — each backed by exactly
    // ONE DB row with a different hit timestamp.  After initProbeCounters we
    // push a sentinel onto KEY_P's hits array and assert KEY_Q's hits array is
    // completely unaffected and still has the correct length.
    const now   = Date.now();
    const hitP  = now - 1_000; // KEY_P single-row timestamp
    const hitQ  = now - 2_000; // KEY_Q single-row timestamp

    const mod    = await import("./traffic-logger");
    const { db } = await import("./db");
    const { _uaProbes } = mod as any;

    const KEY_P = "FreshImportSingleRowUA-P/1.0";
    const KEY_Q = "FreshImportSingleRowUA-Q/1.0";

    // One row each — no duplicates.
    const rowP = { fieldType: "ua", key: KEY_P, hits: [hitP], lastAlerted: 0 };
    const rowQ = { fieldType: "ua", key: KEY_Q, hits: [hitQ], lastAlerted: 0 };

    (db as any).execute = vi.fn().mockResolvedValue([]);
    (db as any).select  = vi.fn().mockReturnValue({
      from: vi.fn().mockResolvedValue([rowP, rowQ]),
    });

    await mod.initProbeCounters();

    // Both UA keys must have been hydrated with exactly 1 hit each.
    const entryP = _uaProbes.get(KEY_P);
    const entryQ = _uaProbes.get(KEY_Q);
    expect(entryP).toBeDefined();
    expect(entryQ).toBeDefined();
    expect(entryP!.hits).toHaveLength(1);
    expect(entryQ!.hits).toHaveLength(1);

    // ── Reference-independence assertion ──────────────────────────────────
    // Push a sentinel onto KEY_P's hits array.  If allHits was declared
    // outside the group loop and reused across groups, the sentinel would
    // silently appear in KEY_Q's hits array too.
    const SENTINEL = 999_111_999_111;
    entryP!.hits.push(SENTINEL);

    // KEY_P should now have 2 elements (1 original + sentinel).
    expect(entryP!.hits).toHaveLength(2);
    expect(entryP!.hits).toContain(SENTINEL);

    // KEY_Q must be completely unaffected — still exactly 1 hit, no sentinel.
    expect(entryQ!.hits).toHaveLength(1);
    expect(entryQ!.hits).not.toContain(SENTINEL);
  });

  it("(B41i) initProbeCounters: allHits buffer is independent between two distinct referer keys when each key has exactly one DB row (no duplicates)", async () => {
    // Referer-map counterpart to B41h.  B41h covers the UA map; this test
    // covers the symmetric case for the referer map.  A merge loop that declares
    // `allHits` outside the group loop would alias the second referer entry's
    // hits array with the buffer left over from the first group iteration,
    // causing a sentinel pushed onto one entry to silently appear in the other.
    //
    // Setup: two distinct referer keys — KEY_P and KEY_Q — each backed by
    // exactly ONE DB row with a different hit timestamp.  After initProbeCounters
    // we push a sentinel onto KEY_P's hits array and assert KEY_Q's hits array
    // is completely unaffected and still has the correct length.
    const now   = Date.now();
    const hitP  = now - 1_000; // KEY_P single-row timestamp
    const hitQ  = now - 2_000; // KEY_Q single-row timestamp
    ...
    const KEY_P = "https://fresh-import-single-row-ref-p.example.com/";
    const KEY_Q = "https://fresh-import-single-row-ref-q.example.com/";

    // One row each — no duplicates.
    const rowP = { fieldType: "referer", key: KEY_P, hits: [hitP], lastAlerted: 0 };
    const rowQ = { fieldType: "referer", key: KEY_Q, hits: [hitQ], lastAlerted: 0 };
    ...
    // Both referer keys must have been hydrated with exactly 1 hit each.
    const entryP = _refererProbes.get(KEY_P);
    const entryQ = _refererProbes.get(KEY_Q);
    expect(entryP).toBeDefined();
    expect(entryQ).toBeDefined();
    expect(entryP!.hits).toHaveLength(1);
    expect(entryQ!.hits).toHaveLength(1);
    ...
    // outside the group loop and reused across groups, the sentinel would
    // silently appear in KEY_Q's hits array too.
    const SENTINEL = 999_222_999_222;
    entryP!.hits.push(SENTINEL);
  ...
  it("(B41j) initProbeCounters: allHits buffer is independent between two distinct referer keys when each key has exactly one DB row and both carry an active cooldown (non-zero lastAlerted)", async () => {
    // Referer-map counterpart to B41i, adding the active-cooldown dimension.
    // B41i seeds both rows with lastAlerted=0.  A merge loop that correctly
    // resets allHits for the zero-lastAlerted path might still forget to reset
    // it when lastAlerted > 0 — for example by only reinitialising the buffer
    // inside an `if (lastAlerted === 0)` branch.  This test exercises that
    // path: both referer rows carry a non-zero lastAlerted (alerted 5 and 10
    // minutes ago, well within the 1-hour default cooldown).
    //
    // Setup: two distinct referer keys — KEY_P and KEY_Q — each backed by
    // exactly ONE DB row with a different hit timestamp and a different
    // non-zero lastAlerted.  After initProbeCounters we push a sentinel onto
    // KEY_P's hits array and assert KEY_Q's hits array is completely
    // unaffected and still has the correct length.
    const now          = Date.now();
    const hitP         = now - 1_000; // KEY_P single-row timestamp
    const hitQ         = now - 2_000; // KEY_Q single-row timestamp
    const lastAlertedP = now - 5  * 60_000; // alerted 5 min ago — active cooldown
    const lastAlertedQ = now - 10 * 60_000; // alerted 10 min ago — active cooldown

    const mod    = await import("./traffic-logger");
    const { db } = await import("./db");
    const { _refererProbes } = mod as any;

    const KEY_P = "https://fresh-import-single-row-ref-cooldown-p.example.com/";
    const KEY_Q = "https://fresh-import-single-row-ref-cooldown-q.example.com/";

    // One row each — no duplicates — both with active lastAlerted values.
    const rowP = { fieldType: "referer", key: KEY_P, hits: [hitP], lastAlerted: lastAlertedP };
    const rowQ = { fieldType: "referer", key: KEY_Q, hits: [hitQ], lastAlerted: lastAlertedQ };
    ...
    // ── Reference-independence assertion ──────────────────────────────────
    // Push a sentinel onto KEY_P's hits array.  If allHits was declared
    // outside the group loop and only reset on the zero-lastAlerted path,
    // the sentinel would silently appear in KEY_Q's hits array too.
    const SENTINEL = 999_333_999_333;
    entryP!.hits.push(SENTINEL);
...
// Cross-key allHits buffer reuse guard — two distinct referer keys (restart path)
//
// Task 598 guards against allHits aliasing between two rows sharing the *same*
// referer key.  A subtler variant is a merge loop that reuses the same allHits
// buffer across *different* referer keys in sequence — for example, by
// declaring allHits outside the per-group loop and forgetting to reset it.
// In that case the second key's entry would contain hits belonging to the first.
//
// These tests seed initProbeCounters with two "referer" rows carrying
// *different* key strings, each with distinct timestamps, and assert that each
// merged entry's hits array contains only its own timestamps and none from the
// other.  This is the symmetric counterpart to the UA cross-key guard below.
// ═══════════════════════════════════════════════════════════════════════════

describe("ProbeEntry cross-key allHits buffer reuse guard (restart path, two distinct referer keys): each entry's hits contains only its own timestamps", () => {
  const KEY_A = "cross-key-referer-allhits-guard-key-a-probe-v1";
  const KEY_B = "cross-key-referer-allhits-guard-key-b-probe-v1";
  ...
  it("entry for KEY_A contains only KEY_A's timestamps and entry for KEY_B contains only KEY_B's timestamps", async () => {
    ...
    // Two "referer" rows with *different* keys — simulates two scrapers present
    // in the DB at restart time.
    const fakeRows = [
      { fieldType: "referer", key: KEY_A, hits: [hitA], lastAlerted: 0 },
      { fieldType: "referer", key: KEY_B, hits: [hitB], lastAlerted: 0 },
    ];
    ...
    // KEY_A's entry must NOT contain KEY_B's timestamp.
    // A future loop that fails to reset allHits between groups would cause
    // KEY_B's entry to absorb hitA, or KEY_A's entry to carry stale data from
    // an earlier iteration — both are caught here.
...
// Cross-key allHits buffer reuse guard — two distinct UA keys (restart path)
//
// Task 598 guards against allHits aliasing between two rows sharing the *same*
// UA key.  A subtler variant is a merge loop that reuses the same allHits
// buffer across *different* UA keys in sequence — for example, by declaring
// allHits outside the per-group loop and forgetting to reset it.  In that
// case the second key's entry would contain hits belonging to the first key.
//
// These tests seed initProbeCounters with two "ua" rows carrying *different*
// key strings, each with distinct timestamps, and assert that each merged
// entry's hits array contains only its own timestamps and none from the other.
// ═══════════════════════════════════════════════════════════════════════════

describe("ProbeEntry cross-key allHits buffer reuse guard (restart path, two distinct UA keys): each entry's hits contains only its own timestamps", () => {
  const KEY_A = "cross-key-ua-allhits-guard-key-a-probe-v1";
  const KEY_B = "cross-key-ua-allhits-guard-key-b-probe-v1";
  ...
  it("entry for KEY_A contains only KEY_A's timestamps and entry for KEY_B contains only KEY_B's timestamps", async () => {
    ...
    // Two "ua" rows with *different* keys — simulates two scrapers present in
...
// an active cooldown (restart path)
//
// Task 620 guards against allHits aliasing between two distinct referer keys
// when both rows carry lastAlerted = 0 (no prior alert).  The symmetric case
// for *UA* keys when both rows have an *active* lastAlerted (within
// ALERT_COOLDOWN_MS) has no dedicated test.  A regression that conflates UA
// entries only when the cooldown branch is taken — for example by declaring
// allHits outside the per-group loop and only resetting it on the
// zero-lastAlerted path — would slip through those tests entirely.
//
// These tests seed initProbeCounters with two "ua" rows carrying *different*
// key strings, each with a distinct timestamp and an active lastAlerted value,
// and assert that each merged entry's hits array contains only its own
...
// ═══════════════════════════════════════════════════════════════════════════
// Cross-key allHits buffer reuse guard — two distinct UA keys, both with
// an active cooldown AND multiple (duplicate) rows per key (restart path)
//
// Task 628 catches the single-row-per-key case.  A subtler regression could
// appear when each key has two or more duplicate rows: the merge loop
// processes multiple rows per group before moving on.  A bug that resets
// allHits between groups but NOT between rows-within-a-group, combined with
// the cooldown branch, would slip through Task 628 entirely.
//
// These tests seed initProbeCounters with two "ua" groups (KEY_A, KEY_B),
// each with two duplicate rows, all four rows carrying active lastAlerted
// values.  They assert:
//   • each merged entry's hits contains only its own timestamps
//   • neither entry's hits contains any timestamp from the other key
//   • the two entries' hits arrays are not the same object reference
// ═══════════════════════════════════════════════════════════════════════════

describe("ProbeEntry cross-key allHits buffer reuse guard (restart path, two distinct UA keys with active cooldown AND multiple rows per key): each entry's hits contains only its own timestamps", () => {
  const KEY_A = "cross-key-ua-allhits-multirow-cooldown-guard-key-a-probe-v1";
  const KEY_B = "cross-key-ua-allhits-multirow-cooldown-guard-key-b-probe-v1";
  ...
  it("entry for KEY_A contains only KEY_A's timestamps and entry for KEY_B contains only KEY_B's timestamps", async () => {
    ...
    // UA is recorded more than once before a restart.
    const fakeRows = [
      { fieldType: "ua", key: KEY_A, hits: [hitA1], lastAlerted: lastAlertedA },
      { fieldType: "ua", key: KEY_A, hits: [hitA2], lastAlerted: lastAlertedA },
      { fieldType: "ua", key: KEY_B, hits: [hitB1], lastAlerted: lastAlertedB },
      { fieldType: "ua", key: KEY_B, hits: [hitB2], lastAlerted: lastAlertedB },
    ];
    (db as any).execute = vi.fn().mockResolvedValue([]);
    (db as any).select  = vi.fn().mockReturnValue({ from: vi.fn().mockResolvedValue(fakeRows) });
